...
export class ConfigEcommerceService {
  constructor(
    ...
    private repository: Repository<ConfigEcommerce>,
    ...
    private repositoryCategoriesFeat: Repository<FeaturedCategory>,
  ...
  async create(createConfigEcommerceDto: CreateConfigDoctorDto) {
    const value = await this.repository.count();
    if (value > 0) {
      throw new HttpException(
        'Ya existe una configuración de doctor creada',
        HttpStatus.CONFLICT,
      );
    }
  ...
  /**
   * Actualiza la información de la configuración del doctor.
   */
  async updateInfoDoctor(update: CreateConfigDoctorDto) {
  ...
  async findWelcomeMessage() {
    ...
    return this.repository.findOne(1, {
      select: ['mensaje_bienvenida', 'banner_bienvenida'],
    });
  }
  ...
  async findOrderMessage() {
    ...
    return this.repository.findOne(1, {
      select: ['mensaje_orden', 'banner_orden'],
    });
  }
  ...
  async findCategories() {
    ...
    let categories = await this.repositoryCategoriesFeat.find({
      relations: ['categoria'],
    });
  ...
  async findCategory(id: string) {
    let val = await this.repositoryCategoriesFeat.findOne(id, {
      relations: ['categoria'],
    });
    if (!val) {
      throw new HttpException(
        'La categoria no es destacada',
        HttpStatus.NOT_FOUND,
      );
    }
    return val;
  ...
  async addCategory(createFeatCategory: CreateFeatCategoryDto) {
    if (
      (await this.repositoryCategoriesFeat.count({
        where: { categoria: { id: createFeatCategory.categoria } },
      })) == 0
    ) {
      return this.repositoryCategoriesFeat.save(createFeatCategory);
    }
    throw new HttpException(
      'La categoría ya es destacada',
      HttpStatus.CONFLICT,
    );
  }
  ...
  async updateCategory(id: string, update: UpdateFeatCategoryDto) {
    return (await this.repositoryCategoriesFeat.update(id, update)).affected > 0
      ? 'Ok'
      : 'Not OK';
  }
  ...
  async addFeaturedProduct(id: string) {
    const count = await this.repositoryProducts.count({
    ...
    if (count < 4) {
      ...
      throw new HttpException(
        'Solo deben existir cuatro productos destacados',
        HttpStatus.CONFLICT,
      );
    }
  }
